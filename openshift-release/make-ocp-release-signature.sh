...
oc create configmap sha256-${DIGEST_TAG_SHASUM} -n openshift-config-managed --from-file=sha256-${DIGEST_TAG_SHASUM}-1=/tmp/ocp-sig-1-${OCP_RELEASE}/signature-1 --dry-run=client -o json \
 | jq '.metadata.labels={'$LABEL_STR'}' | oc apply --dry-run=client -o yaml -f - > /tmp/ocp-sig-1-${OCP_RELEASE}/configmap.yml
