...
6. Create a **Project** under **both** Automation Execution (Controller) and Automation Decision (EDA) - give it a name, point it to the fork of this repo, eg `https://<your-domain>
  ...
  containers:
    - image: >-
      ...
      env:
        ...
        - name: NO_PROXY
          value: >-
            .kemo.labs,.kemo.network,.svc,.local,.cluster,localhost,<ip-address>,<ip-address>/8,<ip-address>/12,<ip-address>/16,<ip-address>
        - name: no_proxy
          value: >-
            .kemo.labs,.kemo.network,.svc,.local,.cluster,localhost,<ip-address>,<ip-address>/8,<ip-address>/12,<ip-address>/16,<ip-address>
      args:
